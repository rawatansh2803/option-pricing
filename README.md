...
Disclaimer - it is not a standard image-based or computer vision project. I have used CNN to basically do option pricing. Options are derivative contracts that give the buyer of the financial contract the right, but not the obligation. In return, buyer has to pay a premium to the seller. Options are very important financial contracts, and they have immense potential in the financial markets, so here we try to use neural networks ( cnn ) for it, it is not a standard image-based or computer vision project. I have used CNN to basically do option pricing. Options are derivative contracts that give the buyer of the financial contract the right, but not the obligation. In return, he has to pay a premium to the seller. Options are very important financial contracts, and they have immense potential in the financial markets, so here we try to . traditionally options are priced through math models like BSM , Heston model , binomial model etc. Here we take a nonparametric approach in which we do not assume the underlying assumptions like no arbitrage, but actually we use the market data itself to get the option pricing  surface. 
